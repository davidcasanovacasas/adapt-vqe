...
def adapt_vqe_basis2(hamiltonian_op, pool, pool2, reference_ket,
    ...
    #---------------------------------------------------------------------
    # Create initial ansatz from basis2
    #---------------------------------------------------------------------
    if len(trial_indices) > 0:
        print("\n Generating initial ansatz beyond HF from basis2")
        for index2 in trial_indices:
            parameters.insert(0,0)
            op_term = pool2.op_index[index2]
            index = pool.op_index.index(op_term)
            ansatz_ops.insert(0,pool.fermi_ops[index])
            ansatz_mat.insert(0,pool.spmat_ops[index])
            print("\n fermi_ops for pool")
            print(pool.fermi_ops[index])


        print("calling tUCCSD")
